 ...
 * @param type const char *		file name for POST multipart/form-data requests, for example "firmware.bin". NULL for normal requests.
 * @param type const char *		content type for POST multipart/form-data requests, for example "application/octet-stream". NULL for normal requests.
 * @param type const char *		content field name for POST multipart/form-data requests, for example "application/octet-stream". NULL for normal requests.
 * @return -1 if no info or > 0 when Content-Length is set by server
 */

int HTTPClient::sendRequest(const char * type, Stream * stream, size_t size, const char * multipartFilename, const char * multipartContentType, const char * multipartFieldName)
{
    ...
    if(multipartFilename)
    {
    	...
    	{
    		strBoundary+=String(mac[i] >> 4,16);
    		strBoundary+=String(mac[i] & 0xF,16);
    	}
    	strBoundary+="";

		strPrefix.reserve(256);
		strPrefix+="--";
		strPrefix+=strBoundary;
		strPrefix+="\r\nContent-Disposition: form-data; name=\"";
		strPrefix+=multipartFieldName?multipartFieldName:"data";
		strPrefix+="\"; filename=\"";
		strPrefix+=multipartFilename;
		strPrefix+="\"\r\n";
		if(multipartContentType)
		{
			strPrefix+="Content-Type: ";
			strPrefix+=multipartContentType;
			strPrefix+="\r\n";
		}
		strPrefix+="\r\n";

		strSuffix.reserve(256);
		strSuffix+="\r\n--";
		strSuffix+=strBoundary;
		strSuffix+="--\r\n";

		lengthAdd = strPrefix.length()+strSuffix.length();
    }

    if(size > 0) {
        addHeader(F("Content-Length"), String(size+lengthAdd));
    }
    ...
    if(buff) {
    	...
    	if(multipartFilename)
    	{
    		...
    		if(!writeWithRetry((const uint8_t *) strPrefix.c_str(),strPrefix.length()))
    		{
                DEBUG_HTTPCLIENT("[HTTP-Client][sendRequest][multipartPrefix] stream write error %d\n", _client->getWriteError());
                free(buff);
                return returnError(HTTPC_ERROR_SEND_PAYLOAD_FAILED);
    		}
        ...
        while(connected() && (stream->available() > 0) && (len > 0 || len == -1)) {
            ...
            if(sizeAvailable) {
        		...
        		if(!writeWithRetry((const uint8_t *) buff, bytesRead))
        		{
        		...
        		{
        			//if success, all bytes were written by writeWithRetry
    	...
    	if(multipartFilename)
    	{
    		...
    		if(!writeWithRetry((const uint8_t *) strSuffix.c_str(),strSuffix.length()))
    		{
                DEBUG_HTTPCLIENT("[HTTP-Client][sendRequest][multipartSuffix] stream write error %d\n", _client->getWriteError());
                free(buff);
                return returnError(HTTPC_ERROR_SEND_PAYLOAD_FAILED);
    		}
...
/*
 * write data to client with retry/sleep. used by multipart sendRequest
 * @return true if successful
 */

bool HTTPClient::writeWithRetry(const uint8_t * data, int len)
{
	int bytesRemaining=len;

	while(connected() && bytesRemaining)
	{
		Serial.printf("Writing %d...\n",bytesRemaining);
		int bytesWritten=_client->write(data, bytesRemaining);
		if(bytesWritten<0 || _client->getWriteError())
		{
            DEBUG_HTTPCLIENT("[HTTP-Client][writeWithRetry] client->write returned %d (error %d)\n", bytesWritten,_client->getWriteError());
			return false;
		}

		if(bytesWritten!=bytesRemaining)
		{
            DEBUG_HTTPCLIENT("[HTTP-Client][writeWithRetry] short write, asked for %d but got %d retry...\n", bytesRemaining, bytesWritten);
		}
		...
		if(bytesRemaining)
		{
			delay(100);	//give the client some time to catch up
		}
	}

	if(!connected())
	{
		DEBUG_HTTPCLIENT("[HTTP-Client][writeWithRetry] connection lost\n");
